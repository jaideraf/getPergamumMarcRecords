...
export default function buildMarcRecord(data, lastID) {
    ...
    subfieldsMap.forEach((obj) => {
        if (parseInt(obj.paragrafo, 10) > 1 && parseInt(obj.paragrafo, 10) < 10) {
            record.append([obj.paragrafo.padStart(3, '0'), obj.subfieldData.replaceAll('#', ' ')]);
        }
        else {
            ...
            record.append([obj.paragrafo.padStart(3, '0'), obj.indicadores, ...newArr]);
        }
    });
    subfieldsMap.clear();
